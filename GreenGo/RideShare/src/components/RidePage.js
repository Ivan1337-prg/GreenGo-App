...
function RidePage()
{
    ...
    const email = location.state?.email || "guest@my.unt.edu";
    const fetchUserRides = useCallback(async () =>
    {
    const res = await fetch(`http://localhost:5100/rides?email=${encodeURIComponent(email)}`);
    ...
    const handleSchedule = async (e) =>
    {
        ...
        try
        {
        const response = await fetch("http://localhost:5100/ride",
        {
            method: "POST",
    ...
    const handleFindMatches = async () =>
    {
        ...
        try
        {
            const response = await fetch(`http://localhost:5100/match?pickup=${encodeURIComponent(pickup)}&dropoff=${encodeURIComponent(dropoff)}`);
            const data = await response.json();
            // const filtered = data.filter((r) => r.email !== email);
            setMatches(data);
        }
        catch (errors)
        {
    ...
    const handleCancelRide = async (id) =>
    {
        ...
        try
        {
        const response = await fetch(`http://localhost:5100/ride/${id}`,
        {
            method: "DELETE",
        });
        const data = await response.json();
